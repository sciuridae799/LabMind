package com.superagent.redisson.servicelock.util;
...
public class ServiceLockKeyEvaluator {
    ...
    public List<String> resolveKeys(Method method, Object[] arguments, String[] keyExpressions) {
        ...
        if (keyExpressions == null || keyExpressions.length == 0) {
            throw new IllegalStateException("ServiceLock.keys must not be empty.");
        }
        ...
        List<String> lockKeys = new ArrayList<>(keyExpressions.length);
        for (String keyExpression : keyExpressions) {
            lockKeys.add(resolveKey(keyExpression, evaluationContext));
        }
        return lockKeys;
    }
    ...
    private String resolveKey(String keyExpression, StandardEvaluationContext evaluationContext) {
        if (!StringUtils.hasText(keyExpression)) {
            throw new IllegalStateException("ServiceLock.keys contains a blank expression.");
        }
        ...
        try {
            ...
            throw new IllegalStateException("Failed to resolve ServiceLock key expression: " + normalizedExpression, ex);
        }
        if (resolvedValue == null) {
            throw new IllegalStateException("ServiceLock key expression resolved to null: " + normalizedExpression);
        }

        String resolvedKey = resolvedValue.toString().trim();
        if (!StringUtils.hasText(resolvedKey)) {
            throw new IllegalStateException("ServiceLock key expression resolved to blank: " + normalizedExpression);
        }
        return resolvedKey;
    }
}
