...
import com.labmind.business.chat.chatagent.config.BusinessChatModelApiConfigProperties;
import com.labmind.business.chat.chatagent.persistence.data.BusinessChatModelApiConfigData;
...
import com.labmind.business.chat.chatagent.persistence.mapper.BusinessChatModelApiConfigMapper;
import com.labmind.business.chat.chatagent.execution.model.BusinessChatModelApiConfigSnapshot;
import com.labmind.business.chat.chatagent.execution.model.BusinessChatModelPricing;
    ...
    private SecretKeySpec apiKeySecretKey() {
        ...
        if (!StringUtils.hasText(keyBase64)) {
            throw new IllegalStateException(
                    "lab-mind.chat.model-api-config.api-key-aes-key-base64 must not be blank");
        }
        byte[] keyBytes;
        try {
            ...
            throw new IllegalStateException(
                    "lab-mind.chat.model-api-config.api-key-aes-key-base64 must be base64 encoded",
                    exception);
        }
        if (keyBytes.length != 16 && keyBytes.length != 24 && keyBytes.length != 32) {
            throw new IllegalStateException(
                    "lab-mind.chat.model-api-config.api-key-aes-key-base64 must decode to 16, 24, or 32 bytes");
        }
